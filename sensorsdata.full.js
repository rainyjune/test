...
sd.initPara = function(para){
  ...
  var ignore_tags = ['mark','/mark','strong','b','em','i','u','abbr','ins','del','s','sup'];
  if(_.isObject(sd.para.heatmap)) {
    sd.para.heatmap.ignore_tags = _.isArray(sd.para.heatmap.ignore_tags)?sd.para.heatmap.ignore_tags:ignore_tags;
...
*/
  ...
  isStyleTag:function(tagname){
    ...
    if(_.indexOf(sd.para.heatmap.ignore_tags,tagname) > -1){
      return true;
    }else{
      return false;
    }
  ...
  initHeatmap : function() {
    ...
    if (sd.para.heatmap.collect_elements === 'all') {
      ...
      _.addEvent(document, 'click', function(e) {
        ...
        }else if(tagName === 'div' && that.isCollectableDiv(target)){
          that.start(ev,target,tagName);
